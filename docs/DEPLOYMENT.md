...
#### `/external add` 测试（需要管理员）
```
/external add vrchat_name:TestUser roles:VIP,Member
```
- [ ] 成功添加外部用户
- [ ] 显示添加信息
- [ ] 验证角色名称

#### `/external list` 测试（需要管理员）
```
/external list
```
- [ ] 显示外部用户列表
...
#### `/external update` 测试（需要管理员）
```
/external update identifier:TestUser roles:VIP
```
- [ ] 成功更新外部用户
- [ ] 显示更新信息

#### `/external remove` 测试（需要管理员）
```
/external remove identifier:TestUser
```
- [ ] 成功删除外部用户
- [ ] 显示删除确认
...
### 外部用户相关问题
1. 外部用户不显示：检查角色名称是否正确
2. 无法添加外部用户：检查角色是否存在于服务器
3. 外部用户头像不显示：正常，使用默认头像（除非关联 Discord 账号）
