...
export async function handleServerSettings(
  ...
  if (interaction.isRepliable() && !interaction.deferred && !interaction.replied) {
    // ModalSubmitInteraction 默认已经 deferred 或 replied? 
    // ButtonInteraction 可能未 reply。
    // 安全起见，只对未处理的 defer。
  ...
  try {
    ...
    const progress = hasManagedRoles ? await calculateBindingProgress(guildId) : { percentage: 0 };

    const embed = new EmbedBuilder()
      .setTitle('Server Management Dashboard')
      .setDescription(statusMsg || 'Overview of server statistics and bot configuration.')
      .setColor(statusMsg?.includes('error') ? EMBED_COLORS.ERROR : EMBED_COLORS.INFO)
 ...
 * 显示配置 Modal
 */
export async function showConfigModal(interaction: ButtonInteraction, guildId: string): Promise<void> {
  const guild = await Guild.findOne({ guildId });
  if (!guild) return;
 ...
 * 处理配置提交
 */
export async function handleServerConfigSubmit(interaction: ModalSubmitInteraction, guildId: string): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  
  const notifyId = interaction.fields.getTextInputValue('notify_user_id');
  const apiStatus = interaction.fields.getTextInputValue('api_status').toLowerCase();
  ...
  try {
    ...
    if (apiStatus.includes('yes') || apiStatus.includes('true') || apiStatus === '1') {
    ...
    await guild.save();
    await handleServerSettings(interaction, 'Configuration updated successfully.');
  } catch (error) {
    await handleCommandError(interaction, error);
  }
}
 ...
 * 执行同步
 */
export async function performSyncNow(interaction: ButtonInteraction, guildId: string): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  ...
  // @ts-ignore
  const { upsertedCount, modifiedCount } = await import('../../utils/database').then(m => m.bulkUpsertDiscordUsers(members, guildId));
  await Guild.updateOne({ guildId }, { lastSyncAt: new Date(), isSyncing: false }); // Ensure syncing state is reset
  ...
  await interaction.editReply({ embeds: [embed] });
  // @ts-ignore
  import('../../utils/logger').then(m => m.logger.info(`Admin ${interaction.user.username} triggered manual sync in ${interaction.guild!.name}`));
}

/**
 * 显示 Role 管理面板
 */
export async function handleRoleManagement(interaction: ButtonInteraction, guildId: string): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  ...
  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setCustomId('server_btn_role_add').setLabel('Add Role').setStyle(ButtonStyle.Success),
    new ButtonBuilder().setCustomId('server_btn_role_clear').setLabel('Clear All').setStyle(ButtonStyle.Danger),
    new ButtonBuilder().setCustomId('server_btn_back').setLabel('Back').setStyle(ButtonStyle.Secondary)
  );
 ...
 * 显示添加 Role 的选择器
 */
export async function handleAddRole(interaction: ButtonInteraction): Promise<void> {
  // @ts-ignore
 ...
 * 处理 Role 选择
 */
export async function handleRoleSelect(interaction: any, guildId: string): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
 ...
 * 清除 Roles
 */
export async function handleClearRoles(interaction: ButtonInteraction, guildId: string): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
 ...
 * 显示 API Key
 */
export async function showApiKey(interaction: ButtonInteraction, guildId: string): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription('**Keep this URL secret!** It grants read access to sponsor data.')
    .addFields(
      { name: 'Endpoint URL', value: `\`http://${process.env.DOMAIN || 'localhost'}:${process.env.PORT || 3000}/api/vrchat/sponsors/${guildId}\`` }
    );
