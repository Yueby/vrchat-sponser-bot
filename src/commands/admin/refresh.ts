...
/**
 * /admin refresh - 清除当前服务器的 API 缓存
 */
export async function handleAdminRefreshCommand(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try {
    ...
    await interaction.reply({ content: '🔴 Failed to clear cache.', flags: MessageFlags.Ephemeral });
  }
}
