...
export async function handleAdminPanel(
  ...
  try {
    ...
    const unboundCount = await VRChatBinding.countDocuments({ guildId, vrchatName: { $exists: false } }); // 实际上这是绑定的文档，应该换种查法
    // 简化统计，只显示 Sponsor 总数
    
    const embed = new EmbedBuilder()
      .setTitle('Administrator Panel')
      .setDescription(statusMsg || 'Select an action below to manage sponsors and users.')
    ...
    const row1 = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId('admin_btn_search').setLabel('Search User').setStyle(ButtonStyle.Primary).setEmoji('🔍'),
      new ButtonBuilder().setCustomId('admin_btn_add').setLabel('Add Sponsor').setStyle(ButtonStyle.Success).setEmoji('➕'),
      new ButtonBuilder().setCustomId('admin_btn_list').setLabel('List All').setStyle(ButtonStyle.Secondary).setEmoji('📋')
    );
    
    const row2 = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId('admin_btn_unbound').setLabel('Check Unbound').setStyle(ButtonStyle.Danger).setEmoji('⚠️'),
      new ButtonBuilder().setCustomId('admin_btn_refresh').setLabel('Refresh Cache').setStyle(ButtonStyle.Secondary).setEmoji('🔄')
    );
 ...
 * 显示 Search Modal
 */
export async function showSearchModal(interaction: ButtonInteraction): Promise<void> {
 ...
 * 处理 Search 提交
 */
export async function handleSearchSubmit(interaction: ModalSubmitInteraction, guildId: string): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    ...
    if (binding) {
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: 'VRChat Name', value: binding.vrchatName || 'Not Bound', inline: true },
          { name: 'Bind Time', value: binding.bindTime ? `<t:${Math.floor(binding.bindTime.getTime()/1000)}:f>` : 'Unknown', inline: false }
        );
      
      const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder().setCustomId(`admin_btn_user_edit_${binding.userId}`).setLabel('Edit').setStyle(ButtonStyle.Primary),
        new ButtonBuilder().setCustomId('admin_btn_back').setLabel('Back to Panel').setStyle(ButtonStyle.Secondary)
      );
 ...
 * 列出所有赞助者
 */
export async function handleViewSponsors(interaction: ButtonInteraction, guildId: string): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
 ...
 * 查看未绑定成员
 */
export async function handleViewUnbound(interaction: ButtonInteraction, guildId: string): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  ...
  if (unboundMembers.length > 0) {
    const list = unboundMembers
      .slice(0, 15)
      .map((m: any, i: number) => `${i + 1}. **${m.displayName}** (<@${m.userId}>)`)
      .join('\n');
 ...
 * 刷新缓存
 */
export async function handleRefreshCache(interaction: ButtonInteraction, guildId: string): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  await interaction.editReply('✅ Cache has been refreshed (User list reloaded).');
  // 重新加载面板
 ...
 * 显示添加用户 Modal
 */
export async function showAddSponsorModal(interaction: ButtonInteraction): Promise<void> {
  const modal = new ModalBuilder()
 ...
 * 处理添加用户提交
 */
export async function handleAddSponsorSubmit(interaction: ModalSubmitInteraction, guildId: string): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  ...
  if (existing) {
    await interaction.editReply(`🔴 User with ID \`${userId}\` already exists.`);
    return;
  }
 ...
 * 显示编辑用户 Modal
 */
export async function showEditSponsorModal(interaction: ButtonInteraction | any, userId: string): Promise<void> {
  ...
  // @ts-ignore
  const binding = await import('../../models/VRChatBinding').then(m => m.default.findOne({ userId, guildId }));
  if (binding) {
 ...
 * 处理编辑提交
 */
export async function handleEditSponsorSubmit(interaction: ModalSubmitInteraction, guildId: string): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  ...
  if (binding && binding.vrchatName !== cleanVrcName) {
     await VRChatBinding.updateOne(
       ...
       { 
         ...
         $push: { nameHistory: { name: binding.vrchatName, changedAt: new Date() } }
       }
     );
  } else if (!binding) {
    // Create if missing
 ...
 * 删除用户
 */
export async function handleDeleteUser(interaction: ButtonInteraction, guildId: string, userId: string): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  ...
  await interaction.editReply(`✅ User <@${userId}> has been removed from sponsors.`);
}
